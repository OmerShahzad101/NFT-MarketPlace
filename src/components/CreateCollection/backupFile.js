...
const CreateCollection = () => {
  ...
  const [collectionData, setCollectionData] = useState(collectionInitialData);
  useEffect(() => {
    getCategories();
  }, []);
  ...
  const onFileChange = (e) => {
    ...
    if (file)
      if (file.type.includes("image")) {
        let { collectionData } = setCollectionData;
        collectionData = { ...collectionData, [e.target.name]: file };
